...
constexpr const char* cfc_map(const char* nuc_seq) {
  // Must use ternary operator because < C++14 allows only a single return statement (and nothing
  // else) in a constexpr
  return ((nuc_seq[0] == 'T' && nuc_seq[1] == 'T' && nuc_seq[2] == 'T')   ? "ACC"
...
std::pair<size_t, size_t> KmerIndex::getECInfo() const {
  ...
  for (const auto& um : dbg) {
    ...
    for (auto& v : vs) {
      ...
      if (cardinality > 0) {
        cardinality_zero = false;
      }
    }
    if (cardinality_zero) {
      cardinality_zero_encounters++;
    }
  }
...
void KmerIndex::BuildTranscripts(const ProgramOptions& opt, std::ofstream& out) {
  ...
  for (auto& fasta : opt.transfasta) {
    std::cerr << "[build] loading fasta file " << fasta << std::endl;
  }
  ...
  if (countNonNucl > 0) {
    std::cerr << "[build] warning: replaced " << countNonNucl
              << " non-ACGUT characters in the input sequence" << std::endl
  ...
  if (countNonAA > 0) {
    std::cerr << "[build] warning: found " << countNonAA
              << " non-standard amino acid characters in the input sequence" << std::endl
              << "        which were reverse translated to 'NNN'" << std::endl;
  }
...
void KmerIndex::BuildDistinguishingGraph(const ProgramOptions& opt, std::ofstream& out) {
  ...
  // Use an external input FASTA file (we'll still need to read it to determine number of targets
  // though and we'll still write out a new FASTA file (TODO: optimize this out)
  ...
  // Begin Shading
  std::set<std::string>
      variants_set;  // Ordered set to keep track of variants (i.e. colors with shades)
  // End Shading
  for (int i = 0; i < opt.transfasta.size(); i++) {  // Currently, this should only be one file
    auto fasta = opt.transfasta[i];
    fp = opt.transfasta.size() == 1 && opt.transfasta[0] == "-" ? gzdopen(fileno(stdin), "r")
                                                                : gzopen(fasta.c_str(), "r");
    seq = kseq_init(fp);
    while (true) {
      l = kseq_read(seq);
      if (l <= 0) {
  ...
  if (num_trans != ncolors) {
    std::cerr << "[build] Detected " << std::to_string(num_trans - ncolors) << " shades"
              << std::endl;
  }
  // End Shading
  ...
  std::cerr << "[build] target de Bruijn graph has k-mer length " << dbg.getK()
            << " and minimizer length " << dbg.getG() << std::endl;
...
void KmerIndex::DListFlankingKmers(const ProgramOptions& opt, const std::string& tmp_file,
  ...
  auto kmers_special =
      kmers;  // copy; kmers_special will be the ones we store as-is (i.e. not flanking); these are
              // the entries where the FASTA name header is not populated
  ...
  auto isInvalidKmer = [](const char* s, const int k) {
    ...
    for (int i = 0; i < k; i++) {
      if (s[i] != 'A' && s[i] != 'T' && s[i] != 'C' && s[i] != 'G') {
        ++count_nonATCG;
        if (count_nonATCG > max_count_nonATCG) {
          return true;
        }
      }
    }
  ...
  auto worker_function = [&, overhang](std::string& seq, bool is_special) {
    ...
    if (is_special) {                   // Sequence has no name; it is "special"
      int seqlen = seq.size() - k + 1;  // number of k-mers
      while (pos < seqlen) {
        if (!isInvalidKmer(seq.c_str() + pos, k)) {
          kmers_.emplace(seq.c_str() + pos);
        }
        ++pos;
      }
      ...
      if (um.isEmpty) {
        ...
        if (lb >= 0 && ub >= lb) {
          for (int i = 0; i < std::min<unsigned long>(lb, overhang); ++i) {
            ...
            if (!isInvalidKmer(seq.c_str() + lb - i, k)) {
              kmers_.emplace(seq.c_str() + lb - i);
            }
          }
        }
        ...
        if (ub > lb && ub + k < seq.length()) {
          for (int i = 0; i < std::min<unsigned long>(seq.length() - ub, overhang); ++i) {
            ...
            if (!isInvalidKmer(seq.c_str() + ub + i, k)) {
              kmers_.emplace(seq.c_str() + ub + i);
            }
          }
        }
  ...
  for (auto& fasta : dlist_fasta_files) {
    ...
    while (true) {
      ...
      if (slen < max_rlen || max_threads_read <= 1 ||
          seq->name.l ==
              0) {  // Small enough (or "special" sequence); no need to create a new thread
        bool is_special = (seq->name.l == 0);
    ...
    for (auto& t : workers) {
      t.join();  // finish up all remaining threads
    }
    gzclose(fp);
    fp = 0;
  }
  ...
  for (const auto& kmer : kmers) {
    ...
    if (!um_.isEmpty) {
      already_in_graph.insert(kmer.rep());
      continue;
    }
    if (!added_dummy_dfk) {
  ...
  for (const auto& kmer : kmers_special) {  // Add these directly (not checking for flanking)
    if (kmers.find(kmer.rep()) != kmers.end()) {
      continue;  // Already found; just continue
    }
    ...
    if (!added_dummy_dfk) {
      ...
      if (um_.isEmpty) {
        ...
        dummy_dfk = kmer.rep();  // for special k-mers, it's ok to make a k-mer already in the graph
                                 // the dummy
        added_dummy_dfk = true;
      }
    }
    kmers.insert(kmer.rep());            // Add to the master k-mer set
    already_in_graph.erase(kmer.rep());  // Erase from here if necessary (because these special
                                         // D-listed k-mers are stringent filters)
    std::string tx_name = "d_list." + std::to_string(N++);
    ++num_trans;
    // target_names_.push_back(tx_name);
    // target_lens_.push_back(k);
  ...
  std::cerr << "[build] identified " << unique_flanking_kmers << " distinguishing flanking k-mers"
            << std::endl;
...
void KmerIndex::BuildEquivalenceClasses(const ProgramOptions& opt, const std::string& tmp_file) {
  ...
  size_t EC_MAX_N_ABOVE_THRESHOLD =
      6000;  // Thresholding ECs to size EC_THRESHOLD will only occur if we encounter
             // >EC_MAX_N_ABOVE_THRESHOLD number of nodes that have size >EC_SOFT_THRESHOLD
  ...
  while (std::getline(infile, line)) {
    ...
    while (proc < seqlen) {
      ...
      } else if (trinfos[n->id].size() == 1 &&
                 trinfos[n->id][0].trid == std::numeric_limits<uint32_t>::max()) {
        continue;
      }
      TRInfo tr;
...
void KmerIndex::PopulateMosaicECs(std::vector<std::vector<TRInfo>>& trinfos) {
  for (const auto& um : dbg) {
    ...
    std::sort(trinfos[n->id].begin(), trinfos[n->id].end(),
              [](const TRInfo& lhs, const TRInfo& rhs) -> bool { return (lhs.trid < rhs.trid); });
...
void KmerIndex::load(ProgramOptions& opt, bool loadKmerTable, bool loadDlist) {
  ...
  if (header_version != INDEX_VERSION) {
    std::cerr << "Error: incompatible indices. Found version " << header_version
              << ", expected version " << INDEX_VERSION << std::endl
  ...
  tmp_size = ((-1ULL >> 1) & tmp_size);  // Mask out MSB (in case we want to use it as a toggle in
  ...
  for (size_t i = 0; i < tmp_size; ++i) {
    ...
    if (opt.threads == 1) {
      ...
      um = dbg.find(kmer);
      if (um.isEmpty) {
        std::cerr << "Error: Corrupted index; unitig not found: " << std::string(buffer)
                  << std::endl;
        exit(1);
      }
        ...
        for (size_t t = 0; t < opt.threads; t++) {
          workers.emplace_back([&, t] {
            for (size_t j = t; j < in_buf_v.size(); j += opt.threads) {
              ...
              um.getData()->deserialize(iss,
                                        !load_positional_info);  // No need to lock because each
                                                                 // node is necessarily unique
            }
          });
        }
        for (auto& t : workers) {
  ...
  std::cerr << "[index] number of targets: "
            << pretty_num(static_cast<size_t>(onlist_sequences.cardinality())) << std::endl;
  std::cerr << "[index] number of k-mers: " << pretty_num(dbg.nbKmers()) << std::endl;
  if (num_trans - onlist_sequences.cardinality() > 0) {
    std::cerr << "[index] number of D-list k-mers: "
              << pretty_num(static_cast<size_t>(num_trans - onlist_sequences.cardinality()))
              << std::endl;
  }
  // Begin Shading
  if (shadeToColorTranscriptMap.size() != 0) {
    ...
    while (getline(in, line)) {
      ...
      while (ss2.good()) {
        ...
        if (tmp_ecval_num < 0 || tmp_ecval_num >= num_trans) {
          std::cerr << "Error: equivalence class file has invalid value: " << tmp_ecval << " in "
...
void KmerIndex::loadTranscriptsFromFile(const ProgramOptions& opt) {
  ...
  std::cerr << "[index] number of targets loaded from file: " << pretty_num(num_trans) << std::endl;
}
...
int KmerIndex::mapPair(const char* s1, int l1, const char* s2, int l2) const {
  ...
  if (um1.getData()->get_mc_contig(um1.dist).second !=
      um2.getData()->get_mc_contig(um2.dist).second) {
    return -1;  // If the mc contigs for um1 and um2 are actually not the same (despite having the
                // same color)
  }
...
void KmerIndex::match(const char* s, int l, std::vector<std::pair<const_UnitigMap<Node>, int>>& v,
                      bool partial, bool cfc) const {
  const Node* n;
  ...
  */
  ...
  if (cfc) {
    ...
    // if (countNonNN > 0) {
    //   std::cerr << "[warning] found " << countNonNN << " non-standard nucleotides in the input
    //   sequence" << std::endl << "           which were translated to 'NNN'" << std::endl;
    // }
    // // reset countNonNN
    // ::countNonNN = 0;
  }
  ...
  for (int i = 0; kit != kit_end; ++i, ++kit) {
    ...
    if (!um.isEmpty) {
      ...
      int dist =
          (forward) ? (contig_length - 1 - (um.dist - contig_start)) : um.dist - contig_start;
      ...
      if (dist >= 2) {
        ...
        if (kit2 != kit_end) {
          ...
          if (found2) {
            ...
            bool is_in_dlist = um2.isEmpty && d_list.size() > 0 &&
                               (d_list.find((kit2->first).rep()) !=
                                d_list.end());  // D-list (check after first jump)
            if (found2pos >= l - k) {
...
// pre:  v is initialized
// post: v contains all equiv classes for the k-mers in s
double KmerIndex::match_long(const char* s, int l,
                             std::vector<std::pair<const_UnitigMap<Node>, int>>& v, bool partial,
                             bool cfc) const {
  const Node* n;
  ...
  while (kit != kit_end) {  // should be + 2?
    ...
    if (um.isEmpty) {
      ...
      if (dist >= 2) {
        ...
        if (kit2 != kit_end) {  //(nextPos < l-k) { //should be +1?
          ...
          if (found2) {
            ...
            if (dist > 4) {
              int middlePos = (pos + nextPos) / 2;
              int middleContig = -1;
              int found3pos = pos + dist;  // middlePos+dist; //formerly pos+dist which is same as
                                           // found2pos, but I think should be middlePos+dist
              KmerIterator kit3(kit);
              kit3 += middlePos - pos;
            ...
            if (!foundMiddle) {
              ++proc;
              ++kit;
              // backup plan, let's play it safe and search incrementally for the rest, until
              // nextStop
              for (int j = 0; kit != kit_end; ++kit, ++j) {
                if (j == skip) {
                  j = 0;
                ...
                if (kit->second >= nextPos) {
                  // continue;
                  // trying out the not breaking case to do a complete incremental search until
                  // kit_end
                  break;
                }
                ++proc;
        ...
        //} //NOTE!!! When this if is outside of the check if nextPos is at the end of the kmer,
        //then we allow for incrementally searching which is the correct behavior?
        else {
          // the sequence is messed up at this point, let's just take the match
          // v.push_back({dbGraph.ecs[val.contig], l-k});
          break;
...
std::pair<int, bool> KmerIndex::mapTranscriptPosition(int tr, uint32_t rawpos,
                                                      const const_UnitigMap<Node>& um, int p,
                                                      bool multipos) const {
  ...
  if (trsense) {
    ...
    ret = {static_cast<int64_t>(trpos + static_cast<int64_t>(um.dist) + 1 - padding),
           csense};  // Case I
    if (!csense) {
      ret.first += k - 1 + p;
    } else {
  ...
  if (trsense) {
    if (csense) {
      ...
      ret = {static_cast<int64_t>(trpos - p + static_cast<int64_t>(um.dist) + 1 - padding), csense};
  // Case I } else { int64_t padding;// = um.size-mc.second+1-k; // kmer position of the end of the
  contig int64_t initial = mc.second;
      ...
      for (int i = ecs.size()-2; i >= 0; i--) {
        ...
        mc = n->get_mc_contig(mc.first-1); // if goes below 0, it'll be cast as the largest unsigned
  int and return the right-most block
      }
