...
std::pair<const_UnitigMap<Node>, int> findFirstMappingKmer(
    const std::vector<std::pair<const_UnitigMap<Node>, int>> &v) {
...
void doStrandSpecificity(Roaring &u, const ProgramOptions::StrandType strand,
                         const std::vector<std::pair<const_UnitigMap<Node>, int32_t>> &v,
                         const std::vector<std::pair<const_UnitigMap<Node>, int32_t>> &v2,
                         bool comprehensive = false) {
  if (comprehensive) {  // Comprehensive means we check every position's strand specificity (not
  ...
  if (!v.empty()) {
    vtmp = Roaring();
    bool firstStrand =
        (strand == ProgramOptions::StrandType::FR);  // FR have first read mapping forward
...
int64_t ProcessBatchReads(MasterProcessor &MP, const ProgramOptions &opt) {
  ...
  std::cerr << "[quant] processed " << pretty_num(numreads) << " reads, " << pretty_num(nummapped)
            << " reads pseudoaligned";
  if (nummapped == 0) {
...
void MasterProcessor::processReads() {
  ...
  if (!opt.batch_mode && !opt.bus_mode) {
    ...
    std::vector<std::thread> workers;
    parallel_bus_read = opt.threads > 4 && opt.files.size() > opt.busOptions.nfiles && !opt.num &&
                        !opt.pseudobam && opt.input_interleaved_nfiles == 0;
    ...
    while (id < num_ids) {
      ...
      parallel_bus_reader_locks.swap(
          mutexes);  // Number of locks = nt = number of batches we're processing in this iteration
      int local_id = 0;
      int start_id = id;
      ...
      for (int i = 0; i < num_threads - nt; i++, local_id++) {  // Use up remaining threads
        workers.emplace_back(std::thread(
            BUSProcessor(index, opt, tc, *this, start_id + (i % nt), local_id)));  // id will cycle
      }
      // let the workers do their thing
...
void MasterProcessor::update(const std::vector<uint32_t> &c, const std::vector<Roaring> &newEcs,
                             ...
                             std::vector<std::pair<Roaring, std::string>> &new_ec_umi, int n,
                             std::vector<int> &flens, std::vector<double> &unmapped_list,
                             std::vector<int> &flens_lr, std::vector<int> &flens_lr_c,
                             std::vector<int> &bias, const PseudoAlignmentBatch &pseudobatch,
                             std::vector<BUSData> &bv,
                             std::vector<std::pair<BUSData, Roaring>> newBP, int *bc_len,
                             int *umi_len, int id, int local_id) {
  ...
  if (opt.pseudobam) {
    ...
    while (true) {
      ...
      auto min_it = std::min_element(
          pseudobatch_stragglers.begin(), pseudobatch_stragglers.end(),
  ...
  if (opt.bus_mode || opt.batch_mode) {
    ...
    if (opt.max_num_reads != 0 &&
        numreads + n > opt.max_num_reads) {  // Downsample current batch of reads while maintaining
                                             // ratio of unmapped/mapped reads
      int bv_size = bv.size();
    ...
    for (auto &bp : newBP) {
      ...
      if (it != bus_ecmapinv.end()) {
        ...
        tc.counts.push_back(
            0);  // Push back zero count (so we can update the EC's actual count later)
        num_new_ecs++;
      }
      auto &b = bp.first;
      b.ec = ec;
  ...
  if (counter >= 1000000) {
    ...
    std::cerr << " (" << std::fixed << std::setw(3) << std::setprecision(1)
              << ((100.0 * nummapped) / double(numreads)) << "% mapped)             ";
    std::cerr.flush();
  }
  //}
  // releases the lock
    ...
    for (int i = 0; i < numSortFiles; i++) {
      ...
      if (i < numSortFiles - 1) {
        ...
        for (uint64_t j = 0; j < n; j++) {
          b = bv[j];
          uint64_t pos = ((uint64_t)b.core.tid) << 32 | ((uint64_t)b.core.pos + 1) << 1 |
                         (b.core.flag & BAM_FREVERSE) >> 4;
          bb.push_back({pos, j});
        }
...
void MasterProcessor::writePseudoBam(const std::vector<bam1_t> &bv) {
  ...
  for (const auto &b : bv) {
    ...
    std::cout << "name: " <<  b.data << ", ldata:" << (int)b.l_data << " ,mdata: " << (int) b.m_data
      <<  ", lqname " << (int) b.core.l_qname <<", lqseq " <<  (int) b.core.l_qseq << ", enull " <<
    (int) b.core.l_extranul << std::endl; for (int i = 0; i < b.l_data; ++i) { std::cout << (int)
    b.data[i] << " ";
    }
    std::cout << std::endl;
    std::cout << "tid: " << b.core.tid << ", pos: " << (int) b.core.pos
              << ", flag: " << b.core.flag << ", ncigar: " << b.core.n_cigar << ", mtid: " <<(int)
    b.core.mtid << ", mpos: " << (int) b.core.mpos << ", isize" << (int) b.core.isize <<  std::endl;
    kstring_t str = { 0, 0, NULL };
...
ReadProcessor::ReadProcessor(const KmerIndex &index, const ProgramOptions &opt,
                             const MinCollector &tc, MasterProcessor &mp, int _id, int _local_id)
    : paired(!opt.single_end && !opt.long_read),
      tc(tc),
      index(index),
      mp(mp),
      id(_id),
...
void ReadProcessor::operator()() {
  while (true) {
    ...
    mp.update(counts, newEcs, ec_umi, new_ec_umi, paired ? seqs.size() / 2 : seqs.size(), flens,
              unmapped_list, flens_lr, flens_lr_c, bias5, pseudobatch, tmp_v,
...
void ReadProcessor::processBuffer() {
  ...
  for (int i = 0; i < seqs.size(); i++) {
    ...
    if (u.isEmpty()) {
      if (mp.opt.fusion && !(v1.empty() || v2.empty())) {
        ...
        if (findFusion) {
          ...
          searchFusion(index, mp.opt, tc, mp, n1, s1, v1, n2, s2, v2, paired);
        }
      }
      novel = true;
    }
    ...
    if (!novel || !mp.opt.long_read) {
      // If we have paired end reads where one end maps or single end reads, check if some
      // transcsripts are not compatible with the mean fragment length
      if (!mp.opt.single_overhang && !u.isEmpty() && (!paired || v1.empty() || v2.empty()) &&
          tc.has_mean_fl) {
        vtmp = Roaring();
      ...
      if (!u.isEmpty()) {
        ...
        if (elem == index.ecmapinv.end()) {
          ...
          newEcs.push_back(u);
                   ...
                   counts.size()) {  // handle race condition (if counts isn't updated yet)
          newEcs.push_back(u);
        } else {
          // add to count vector
          ++counts[elem->second];
        ...
        if (!mp.opt.long_read && findFragmentLength && flengoal > 0 && paired &&
            u.cardinality() == 1 && !v1.empty() && !v2.empty()) {
...
BUSProcessor::BUSProcessor(/*const*/ KmerIndex &index, const ProgramOptions &opt,
                           const MinCollector &tc, MasterProcessor &mp, int _id, int _local_id)
    : paired(!opt.single_end),
      bam(opt.bam),
      num(opt.num),
      tc(tc),
      index(index),
  ...
  while (true) {
    ...
    if (mp.opt.batch_mode) {
      ...
      if (mp.FSRs[SRindex].empty()) {
        ...
        mp.FSRs[SRindex].fetchSequences(buffer, bufsize, seqs, names, quals, flags, umis,
                                        readbatch_id, mp.opt.pseudobam,
                                        mp.opt.busOptions.keep_fastq_comments);
      }
      ...
      mp.FSRs[i].fetchSequences(buffer, bufsize, seqs, names, quals, flags, umis, readbatch_id,
                                mp.opt.pseudobam || mp.opt.fusion,
                                mp.opt.busOptions.keep_fastq_comments);
    } else {
      std::lock_guard<std::mutex> lock(mp.reader_lock);
    ...
    mp.update(counts, newEcs, ec_umi, new_ec_umi, seqs.size() / mp.opt.busOptions.nfiles, flens,
              unmapped_list, flens_lr, flens_lr_c, bias5, pseudobatch, bv, std::move(newB),
              &bc_len[0], &umi_len[0], id, local_id);
    clear();
...
void BUSProcessor::processBuffer() {
  ...
  bool bulk_like = (mp.opt.batch_mode && no_technology) ||
                   busopt.umi[0].fileno == -1;  // Treat like bulk: no UMI
  ...
  for (int i = 0; i + incf < seqs.size(); i++) {
    ...
    if (check_tag_sequence) {
      ...
      if (hamming(tag_binary, umi_binary >> 2 * (ulen - taglen), taglen) <=
          (taglen <= 5 ? 0 : hamming_dist)) {  // if tag present (hamming_dist threshold kicks in
                                               // when taglen > 5)
    ...
    if (singleSeq) {
      int seqstart =
          (ignore_umi && busopt.umi[0].fileno == seqopt.fileno ? busopt.umi[0].start - taglen
                                                               : seqopt.start);
      ...
      int seqstart1 =
          (ignore_umi && busopt.umi[0].fileno == sopt1.fileno ? busopt.umi[0].start - taglen
                                                              : sopt1.start);
      int seqstart2 =
          (ignore_umi && busopt.umi[0].fileno == sopt2.fileno ? busopt.umi[0].start - taglen
                                                              : sopt2.start);
      ...
      for (int j = 0; j < busopt.seq.size(); j++) {
        ...
        int seqstart =
            (ignore_umi && busopt.umi[0].fileno == sopt.fileno ? busopt.umi[0].start - taglen
                                                               : sopt.start);
    ...
    if (mp.opt.batch_mode && no_technology) {
      ...
      getFragLenIfPaired = true;
      blen = BUSFORMAT_FAKE_BARCODE_LEN;
      memcpy(bc, binaryToString(mp.batch_id_mapping[id], blen).c_str(),
             blen);  // Create fake barcode that identifies the batch
    } else if (mp.opt.batch_mode && !no_technology && mp.opt.record_batch_bus_barcode &&
               busopt.bc[0].fileno == -1) {
    ...
    if (mp.opt.batch_mode && !no_technology && mp.opt.record_batch_bus_barcode &&
        busopt.bc[0].fileno != -1) {
    ...
    if (!novel || !mp.opt.long_read) {
      ...
      if (doStrandSpecificityIfPossible && mp.opt.strand_specific &&
          !u.isEmpty()) {  // Strand-specificity
        bool comprehensive = false;
        if (mp.opt.do_union || mp.opt.no_jump) {
          comprehensive = true;
        }
 ...
 newB.push_back({b, u});
      ...
      // now address reads that are considered novel and need to be written to novel fastq and
      // processed later.
      std::stringstream ss;
      if (u.isEmpty()) {
        std::string s(seq);
...
#ifndef NO_HTSLIB
AlnProcessor::AlnProcessor(const KmerIndex &index, const ProgramOptions &opt, MasterProcessor &mp,
                           const EMAlgorithm &em, const Transcriptome &model, bool useEM, int _id)
  ...
  while (true) {
    ...
    if (mp.opt.batch_mode) {
      ...
      if (batchSR.empty()) {
        ...
        assert(pseudobatch.aln.size() ==
               ((paired) ? seqs.size() / 2 : seqs.size()));  // sanity checks
      }
    } else {
      std::lock_guard<std::mutex> lock(mp.reader_lock);
      if (mp.SR->empty()) {
        ...
        if (mp.opt.bus_mode) {
          assert(pseudobatch.aln.size() ==
                 seqs.size() / mp.opt.busOptions.nfiles);  // sanity checks
        } else {
          assert(pseudobatch.aln.size() ==
                 ((paired) ? seqs.size() / 2 : seqs.size()));  // sanity checks
        }
...
void AlnProcessor::processBufferTrans() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    fillBamRecord(b1, nullptr, seqs[si1].first, names[si1].first, quals[si1].first,
                  seqs[si1].second, names[si1].second, pi.r1empty, trans_auxlen);
    if (paired) {
      fillBamRecord(b2, nullptr, seqs[si2].first, names[si2].first, quals[si2].first,
                    seqs[si2].second, names[si2].second, pi.r2empty, trans_auxlen);
    }
    ...
    if (pi.r1empty && pi.r2empty) {
      ...
      if (ua.empty()) {
        ...
        auto strandednessInfo =
            [&](Kmer km, const_UnitigMap<Node> &val,
                const std::vector<std::pair<int, double>> &ua) -> std::pair<bool, bool> {
...
void AlnProcessor::processBufferGenome() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (!mp.opt.bus_mode) {
      // fill in the data info
      fillBamRecord(b1, nullptr, seqs[si1].first, names[si1].first, quals[si1].first,
                    seqs[si1].second, names[si1].second, pi.r1empty, genome_auxlen);
      // b1.id = idnum++;
      if (paired) {
        fillBamRecord(b2, nullptr, seqs[si2].first, names[si2].first, quals[si2].first,
                      seqs[si2].second, names[si2].second, pi.r2empty, genome_auxlen);
        // b2.id = idnum++;
      }
    } else {
      bool tag_present =
          !mp.opt.tagsequence.empty() && pi.UMI != -1;  // tag+UMI present in the sequence
      bool umi_first_file =
          mp.opt.busOptions.umi[0].fileno ==
          mp.opt.busOptions.seq[0].fileno;  // UMI-containing file is the first file (file number 0)
      int tagseqstart;
      if (tag_present) {
        tagseqstart =
            umi_first_file
                ? mp.opt.busOptions.seq[0].start
                : mp.opt.busOptions.seq[1]
                      .start;  // Position where the actual sequence begins when tag+UMI present
      }
      ...
      if (tag_present && umi_first_file) {
        ...
        fillBamRecord(b1, nullptr, seqs[si1].first + tagseqstart, names[si1].first,
                      quals[si1].first + tagseqstart, seqs[si1].second - tagseqstart,
                      names[si1].second, pi.r1empty, genome_auxlen);
      } else {
        fillBamRecord(b1, nullptr, seqs[si1].first, names[si1].first, quals[si1].first,
                      seqs[si1].second, names[si1].second, pi.r1empty, genome_auxlen);
      }
      if (paired) {
        if (tag_present && !umi_first_file) {
          ...
          fillBamRecord(b2, nullptr, seqs[si2].first + tagseqstart, names[si2].first,
                        quals[si2].first + tagseqstart, seqs[si2].second - tagseqstart,
                        names[si2].second, pi.r2empty, genome_auxlen);
        } else {
          fillBamRecord(b2, nullptr, seqs[si2].first, names[si2].first, quals[si2].first,
                        seqs[si2].second, names[si2].second, pi.r2empty, genome_auxlen);
        }
    ...
    if (pi.r1empty && pi.r2empty) {
      ...
      if (ua.empty()) {
        ...
        auto strandednessInfo = [&](Kmer km, const_UnitigMap<Node> &val,
                                    const std::vector<std::pair<int, double>> &ua,
                                    std::vector<uint32_t> &trs) -> std::pair<bool, bool> {
          val = index.dbg.find(km);
          bool reptrue = (km == km.rep());
          if (val.isEmpty) {
            return {false, reptrue};
        ...
        for (auto tp : ua) {
          ...
          auto findBestPos = [&](const std::vector<std::pair<int, bool>> &pv, int tr,
                                 const_UnitigMap<Node> &um, std::pair<const char *, int> seq,
                                 int kpos) -> std::pair<int, bool> {
        ...
        for (auto &x : alnmap) {
          ...
          if (!pi.r1empty) {
            ...
            b1c.core.bin =
                hts_reg2bin(b1c.core.pos, b1c.core.pos + seqs[si1].second - seq1_offset - 1, 14, 5);
            b1c.core.qual = 255;
            if (useEM) {
              memcpy(b1c.data + b1c.l_data - 4, &prob, 4);  // set ZW tag
            }
          ...
          if (paired) {
            ...
            if (!pi.r2empty) {
              ...
              b2c.core.bin =
                  hts_reg2bin(b2c.core.pos, b2c.core.pos + seqs[si2].second - seq2_offset, 14, 5);
              b2c.core.qual = 255;
              if (useEM) {
                memcpy(b2c.data + b2c.l_data - 4, &prob, 4);
              }
...
int fillBamRecord(bam1_t &b, uint8_t *buf, const char *seq, const char *name, const char *qual,
                  int slen, int nlen, bool unmapped, int auxlen) {
  ...
  if (buf == nullptr) {
    ...
    int blen = b.core.l_qname + 16 + ((b.core.l_qseq + 2) >> 1) + b.core.l_qseq +
               auxlen;  // 16 for cigar, auxlen for auxiliary fields
...
bool FastqSequenceReader::empty() {
  return (!state &&
          (current_file >= files.size() || (interleave_nfiles != 0 && current_file >= 1)));
}
...
bool FastqSequenceReader::fetchSequences(char *buf, const int limit,
                                         std::vector<std::pair<const char *, int>> &seqs,
                                         std::vector<std::pair<const char *, int>> &names,
                                         std::vector<std::pair<const char *, int>> &quals,
                                         std::vector<uint32_t> &flags,
                                         std::vector<std::string> &umis, int &read_id, bool full,
                                         bool comments) {
  std::string line;
  std::string umi;
    ...
    if (all_l) {
      ...
      if (bufpos + bufadd < limit) {
        ...
        for (int i = 0; i < nfiles; i++) {
          ...
          if (full && !comments) {
            ...
            if (umi_pos != nullptr) {
              ...
              umis.emplace_back(umi_extra_length == 0 ? std::string(umi_pos + 5)
                                                      : std::string(umi_pos + 5, umi_extra_length));
            }
          }
        }
        ...
        if (interleave_nfiles != 0) {
          std::cerr << "Error: There was an error processing interleaved FASTQ input. Exiting..."
                    << std::endl;
          exit(1);
        }
...
// returns true if there is more left to read from the files
bool BamSequenceReader::fetchSequences(char *buf, const int limit,
                                       std::vector<std::pair<const char *, int>> &seqs,
                                       std::vector<std::pair<const char *, int>> &names,
                                       std::vector<std::pair<const char *, int>> &quals,
                                       std::vector<uint32_t> &flags, std::vector<std::string> &umis,
                                       int &read_id, bool full, bool comments) {
  readbatch_id += 1;       // increase the batch id
  read_id = readbatch_id;  // copy now because we are inside a lock
  seqs.clear();
  flags.clear();
